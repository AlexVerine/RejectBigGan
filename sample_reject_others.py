...
def run_samplerD(config):
  ...
  Sampling, M = get_sampling_function(config, loaders, D)
  Sampling.eval()
  ...
  if config['sample_inception_metrics']: 
    ...
    if x.shape[0]>= config['num_pr_images']:
      ...
      Psi, Rsi, Psa, Rsa = get_pr_curve(x[:config['num_pr_images']], name)
    else:
      P, R, Psi, Rsi, Psa, Rsa, D, C = 0, 0, 0, 0, 0, 0
    ...
    utils.write_evaldata(config['logs_root'], experiment_name, config, 
                         {'itr': state_dict['itr'], 'IS_mean':IS_mean, 'IS_std' : IS_std, 'FID':FID, 
                          'P':P, 'R':R, 'Psa':Psa, 'Rsa':Rsa,  'Psi':Psi, 'Rsi':Rsi,"D":D, "C":C,
                            'rate':Na/N,  'sampling': config['sampling'],'gamma': config['gamma_drs'], 'budget': config['budget'] })
